...
public sealed class ProductListViewModel : BaseViewModel
{
    ...
    public async Task LoadProductsAsync()
    {
        ...
        {
            ErrorMessage = $"Unable to load products. Confirm the API is running. Android emulator uses http://10.0.2.2:5000 and Windows uses http://localhost:5000. Details: {ex.Message}";
        }
        finally
        {
            IsBusy = false;
